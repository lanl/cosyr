...
run_name = "test_beam_remap_g1000_adap_grad"
...
## load wavelets
wavelet_x, wavelet_y, wavelet_field = gen_test_wavelets(scaled_alpha*2, scaled_chi*2, np.int((npt_alpha-1)*wpc[0]*2+2), np.int((npt_chi-1)*wpc[1]*2+1), gamma, unscale_coord=True, distribution="gradient_1d", method="potential_far_field", filter=True, filter_exp="np.abs(wx) > 300.0")
if (mpi_rank==0) :
